...
- `idx_status_created (status, created_at DESC)`：按状态列出话题，支撑分析接口读取正常话题集合。
...
- `idx_created_type (created_at, interaction_type)`：互动类型占比分析。
...
- `idx_user_created (user_id, created_at)`：用户行为时间线。
- `idx_user_topic_valid_created (user_id, topic_id, is_valid, created_at)`：用户同话题有效行为计数。
- `idx_topic_created (topic_id, created_at)`：话题行为时间线。
- `idx_device (device_fingerprint)`：设备指纹基础检索。
- `idx_behavior_device_created_user (device_fingerprint, created_at, user_id)`：行为表中的设备关联用户检测。
- `idx_valid_created_topic (is_valid, created_at DESC, topic_id)`：反作弊报告和 Day5 反作弊统计。
...
```sql
SELECT interaction_type, COUNT(*) AS total_count
FROM interaction_event
WHERE created_at >= ?
  AND created_at < ?
...
```sql
SELECT id, user_id, topic_id, interaction_type, device_fingerprint, ip_address,
